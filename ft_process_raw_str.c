...
//!! length 는 보류 !!
	...
	if (ft_strchr("cs\%", spc))
	{
		...
		{
			ft_strlcpy(res, raw_str, raw_strlen);
			ft_memset(&res[raw_strlen], ' ', padding);
		}
		else if (padding > 0)
		{
			ft_memset(res, ' ', padding);
			ft_strlcpy(&res[padding], raw_str, raw_strlen);
		}
		else
			ft_strlcpy(res, raw_str, raw_strlen);
	}
	else if (ft_strchr("pdiuxX", spc))
	{
		if (padding > 0 && ft_strchr(flag, '-'))
		{
			...
			ft_strlcpy(&res[i], raw_str, raw_strlen);
			i += raw_strlen;
			if (ft_strchr(flag, '0') ||
